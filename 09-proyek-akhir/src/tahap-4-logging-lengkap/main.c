...
/* LED status: mati saat idle, menyala solid saat logging aktif. */
static void perbarui_led_status(bool aktif)
{
	gpio_pin_set_dt(&led, aktif ? 1 : 0);
}
...
static void sensor_thread_fn(void *p1, void *p2, void *p3)
{
	...
	if (!device_is_ready(sensor)) {
		LOG_ERR("Sensor MPU6050 tidak siap, thread sensor berhenti");
		return;
	}
	...
	while (1) {
		if (atomic_get(&logging_aktif)) {
			...
			if (ret == 0) {
				...
				LOG_ERR("Gagal fetch sample sensor: %d", ret);
			}
		}
...
static void logger_thread_fn(void *p1, void *p2, void *p3)
{
	...
	while (1) {
		...
		LOG_INF("[%lld ms] accel x=%d.%06d y=%d.%06d z=%d.%06d m/s^2",
...
static int cmd_start(const struct shell *sh, size_t argc, char **argv)
{
	...
	perbarui_led_status(true);
	shell_print(sh, "Logging diaktifkan");
	LOG_INF("Logging diaktifkan lewat shell");
	return 0;
}
...
static int cmd_stop(const struct shell *sh, size_t argc, char **argv)
{
	...
	perbarui_led_status(false);
	shell_print(sh, "Logging dihentikan");
	LOG_INF("Logging dihentikan lewat shell");
	return 0;
}
...
int main(void)
{
	if (!gpio_is_ready_dt(&led)) {
		LOG_ERR("LED status tidak siap");
		return 0;
	}
	gpio_pin_configure_dt(&led, GPIO_OUTPUT_INACTIVE);

	LOG_INF("Data logger siap. Ketik 'datalogger start' di shell untuk mulai.");
	return 0;
}
